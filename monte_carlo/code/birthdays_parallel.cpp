...
int main()
{
    ...
    for (int max_attempts = 1; max_attempts < 30; ++max_attempts)
    {
        ...
        safe_object<int> num_common_birthdays_ts(0);
        ...
        hpx::for_each(hpx::execution::par, hpx::util::counting_iterator(0),
            hpx::util::counting_iterator(n_total), [&](auto) {
                ...
                {
                    ++num_common_birthdays_ts.get();
                }
            });

        int num_common_birthdays = num_common_birthdays_ts.reduce(
            [](int acc, int val) { return acc + val; });

        double probability_of_common_birthdays =
            double(num_common_birthdays) / n_total;
        std::println("{},{},{},{:%S}", n_total, probability_of_common_birthdays,
